...
new_df = pd.read_csv('test.csv', index_col='Unnamed: 0')
...
for i in range(45):
    ...
    b_plus.append((min_b + max_b)/2)
    b_minus.append((max_b-min_b)/2)

bo = np.array(b_plus)
new_df['b_row_plus'] =  pd.Series(bo, index=new_df.index)

bo1 = np.array(b_minus)
new_df['b_row_minus'] =  pd.Series(bo1, index=new_df.index)

b_1 = np.array([b1 for i in range(45)])
b11 = pd.Series(b_1, index=new_df.index)
new_df['b1'] =  pd.Series(b11, index=new_df.index)

b_2 = np.array([b2 for i in range(45)])
b12 = pd.Series(b_2, index=new_df.index)
new_df['b2'] =  pd.Series(b12, index=new_df.index)


b_3 = np.array([b3 for i in range(45)])
b13 = pd.Series(b_3, index=new_df.index)
new_df['b3'] =  pd.Series(b13, index=new_df.index)


b_4 = np.array([b4 for i in range(45)])
b14 = pd.Series(b_4, index=new_df.index)
new_df['b4'] =  pd.Series(b14, index=new_df.index)



new_df.to_csv('table_51_f_n1.csv')
...
#                        (-lx33_1) * t_0(new_df['x33'].iloc[i]) - lx33_2 * t_1(new_df['x33'].iloc[i]) - lx33_3 * t_2(new_df['x33'].iloc[i]) + l_facke - new_df['Y1'].iloc[i] >= 0, str(i + 1)
#
#
# for i in range(32):
#     problem += y[i] == lx11_1 * t_0(new_df['x11'].iloc[i]) + lx11_2 * t_1(new_df['x11'].iloc[i]) + lx11_3 * t_2(new_df['x11'].iloc[i]) + \
...
#                        lx31_1 * t_0(new_df['x31'].iloc[i]) + lx31_2 * t_1(new_df['x31'].iloc[i]) + lx31_3 * t_2(new_df['x31'].iloc[i]) + \
#                        lx32_1 * t_0(new_df['x32'].iloc[i]) + lx32_2 * t_1(new_df['x32'].iloc[i]) + lx32_3 * t_2(new_df['x32'].iloc[i]) + \
#                        lx33_1 * t_0(new_df['x33'].iloc[i]) + lx33_2 * t_1(new_df['x33'].iloc[i]) + lx33_3 * t_2(new_df['x33'].iloc[i]) + l_facke + new_df['Y1'].iloc[i] >= 0, str(i + 46)
#
#
#
#
